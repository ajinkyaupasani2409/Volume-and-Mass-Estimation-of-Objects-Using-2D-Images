...
public class EdgeDetection1 {
	public static void writeWidthOfFile(File f) {
		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);

		Mat im = Imgcodecs.imread(f.getPath()), edges = new Mat(), im2 = new Mat();
		...
		Imgcodecs.imwrite("DeNoisedImage2.jpg", im);
		Imgproc.Canny(im, edges, 20, 100);

		Imgcodecs.imwrite("EdgeDetectedImage2.jpg", edges);

		BufferedImage image = null;

		try {
			image = ImageIO.read(new File("EdgeDetectedImage2.jpg"));
		} catch (IOException e) {
